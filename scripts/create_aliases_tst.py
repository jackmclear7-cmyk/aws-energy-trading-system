#!/usr/bin/env python3
"""
Create Agent Aliases using TSTALIASID

This script creates test aliases for all Bedrock agents.
"""
...
class TestAliasManager:
    """Test alias manager for Bedrock agents"""
    ...
    def create_test_alias(self, agent_id, agent_name):
        """Create a test alias for the agent"""
        try:
            logger.info(f"Creating test alias for agent {agent_name}...")
            
            response = self.bedrock_agent_client.create_agent_alias(
                agentId=agent_id,
                agentAliasName="TSTALIASID",
                description=f"Test alias for {agent_name}"
            )
            
            alias_id = response['agentAlias']['agentAliasId']
            logger.info(f"✅ Created test alias (ID: {alias_id}) for agent {agent_name}")
            return alias_id
            
        except ClientError as e:
            if e.response['Error']['Code'] == 'ConflictException':
                logger.warning(f"⚠️ Test alias already exists for agent {agent_name}")
                # Get existing alias
                try:
                    ...
                    alias_id = response['agentAlias']['agentAliasId']
                    logger.info(f"✅ Using existing test alias (ID: {alias_id}) for agent {agent_name}")
                    return alias_id
                except ClientError as get_error:
                    logger.error(f"❌ Failed to get existing alias for agent {agent_name}: {get_error}")
                    return None
            else:
                logger.error(f"❌ Failed to create test alias for agent {agent_name}: {e}")
                return None
    
    def setup_all_test_aliases(self):
        """Setup test aliases for all agents"""
        
        logger.info("🏷️ Creating Test Aliases for Bedrock Agents")
        logger.info("=" * 60)
        ...
        for agent_name, agent_id in all_agents.items():
            ...
            # Create test alias
            alias_id = self.create_test_alias(agent_id, agent_name)
            if not alias_id:
        ...
        with open('bedrock_agent_test_aliases.json', 'w') as f:
            json.dump(agent_info, f, indent=2)
        
        logger.info(f"\n📄 Agent test alias information saved to: bedrock_agent_test_aliases.json")
        logger.info(f"🎉 Successfully set up test aliases for {len(agent_info)} agents")
...
def main():
    """Main setup function"""
    print("🏷️ Creating Test Aliases for Bedrock Agents")
    print("=" * 60)
    
    manager = TestAliasManager()
    
    try:
        agent_info = manager.setup_all_test_aliases()
        
        if agent_info:
            print(f"\n✅ Successfully set up test aliases for {len(agent_info)} agents:")
            print("-" * 60)
            ...
            print("🎯 All agents are now ready for testing!")
            print("   You can invoke agents using their test alias IDs.")
        else:
            print("❌ No agent test aliases were created successfully")
